...
运营商开通了动态公网IP，**入户光猫是桥接模式（需运营商配合开通超级管理员）**。直连软路由，**软路由负责PPPoE拨号上网**，期待软路由做一些科学上网，DDNS，签到等工作。然后直连一个交换机，把信号分到各个屋里，其中一根线走到无线路由器供家中无线设备连接。**无线路由器是原生固件，设置成有线中继模式**，也叫AP模式，只负责用来发射WiFi信号。

这样设置的好处是：整个网络会在一个局域网下，**有且只有软路由这一个网关**。所有设备都是互通互联的。不会存在多级网络。
...
- 光猫：天邑TEWA-600AGM   -> **桥接网络**；只负责光纤的调制解调；如果开通IPTV，可能需要改回PPPoE上网模式。
...
- 软路由：J1900的一个小机器  -> **PPPoE拨号上网；DHCP分发；网关；科学上网；DDNS**。使用ImmortalWrt固件；**唯一网关**；
  
  - 端口eth0：WAN口连接入户光猫
  
  - 端口eth1：LAN口连接交换机

- 交换机：H3C 8口千兆  -> 负责设备有线网络分发；
...
- 无线路由器：小米AX3600 -> **中继模式**；负责设备无线网络分发。
...
> 因为iStoreOS的OpenClash经常性打不开，导致iStoreOS的主页也打不开，进不到后台中，所以就重新刷一个固件。iStoreOS对x86的支持不好，能用的固件只更新到22年，所以选了一个新的固件ImmortalWrt，这个是比较纯净的Openwrt的扩展版，比Openwrt能好操作一些，比iStoreOS更稳定，且内核版本也很新。

#### 整体是跟着 YouTube的悟空的教程做的

Youtube上有教程，比较简单，目的是用github的action编译一个附带docker，istore，的ImmortalWrt固件。

[GitHub - wukongdaily/AutoBuildImmortalWrt](https://github.com/wukongdaily/AutoBuildImmortalWrt)

1. 在这个github上fork到本地一个，然后进入Action，根据情况选择要编译的包。 我的是X86/64, 版本选的是25.10.4 ；内存留2GB，这个应该是overlay中存软件的地方。勾选了Docker和iStore。

2. 编译好的固件是.gz格式的。immortalwrt-24.10.4-x86-64-generic-squashfs-combined-efi.img.gz 这样的。其中squashfs的意思是，内核文件不可更改，更加稳定，所有修改都是在overlay上做的操作。etf4 的话就是普通版本。

3. 将固件刷到硬盘中：
   
   1. 可以使用U盘刷入，但是需要pe启动输入，很麻烦。
   
   2. 因为我可以直接往硬盘中刷入，所以我就直接往ssd硬盘刷入了，连接硬盘到mac电脑，使用Ether将固件刷入。

4. 刷完固件后，就可以开机接入网络了，默认的地址是192.168.100.1。这个可以在build 固件的时候更改 ，如果有需要的话。

5. 进去后需要先设置一个root的密码。

6. 在网络->接口中找到wan口进行编辑，**这里是编辑拨号上网，还是直连路由器上网。** 或者其他方式。

7. 因为已经直接集成openclash了，所以不需要再手动安装了，更新内核，接入订阅即可。
